...
\subsubsection{Definition: kernel $H$ forms $G/H$}
\begin{definition}
...
\subsubsection{Normal Subgroup $H\lhd G: aH=Ha,\forall a\in G$}
\begin{definition}
...
\subsubsection{Theorem: Well-defined Left Cosets Multiplication $\Leftrightarrow$ Normal}
\begin{theorem}
...
\subsubsection{Corollary: normal subgroup $H$ forms $G/H$}
\begin{corollary}
...
\subsubsection{Theorem: every normal subgroup can be a kernel of a surjective homomorphism $\gamma: G \rightarrow G/H$}
...
\subsubsection{The Fundamental Homomorphism Theorem: Every homomorphism $\phi$ can be factored to a homomorphism $\gamma: G \rightarrow G/H$ and isomorphism $\mu : G/H \rightarrow	\phi[G]$}
\begin{center}\begin{figure}[htbp]
    \centering
...
\begin{theorem}[The Fundamental Homomorphism Theorem]
    \quad
    \begin{center}
        \fcolorbox{black}{gray!10}{\parbox{.9\linewidth}{\underline{Homomorphism $\phi: G \rightarrow G'$ with kernel $H$} can be \textbf{factored} $$\phi=\mu\gamma$$ where $\gamma: G \rightarrow G/H$ is a \underline{homomorphism}, $\mu : G/H \rightarrow	\phi[G]$ is an \underline{isomorphism}}}
    \end{center}
...
\begin{definition}
    A group is \underline{simple} if it is nontrivial and has no proper nontrivial normal subgroups.
\end{definition}
\begin{theorem}
...
\begin{definition}
    ...
    The automorphism $\phi_g : G \rightarrow G$, where $\phi_g(x) = gxg^{-1}$ for all $x \in G$, is the \underline{inner automorphism} of $G$ by $g$.\\
    Performing $\phi_g$ on $x$ is called \underline{conjugation} of $x$ by $g$.
\end{definition}
