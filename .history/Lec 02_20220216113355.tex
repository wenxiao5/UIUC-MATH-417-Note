...
\begin{enumerate}
    \item $aH=bH$ if and only if $a^{-1}bH$
...
\begin{theorem}[Order of element/cyclic group]
For $a\in G$, the order of $a$ (the smallest $m$ such that $a^m=e$) divides $|G|$. The order of $a$ is the order of cyclic group $\left\langle a\right\rangle$ with generator $a$.
\end{theorem}
\begin{proof}
\quad\\
...
\subsection{Homomorphism, Isomorphism}
\subsubsection{Definition}
\begin{definition}
...
If $f$ is also a bijection, then $f$ is called an \textbf{isomorphism}. We say that $G$ and $H$ are \textbf{isomorphic} if exists an \textbf{isomorphism} $f$, denoted by $G\cong H$. (since $f$ is bijection, $G\cong H\Leftrightarrow H\cong G$)
\end{definition}
