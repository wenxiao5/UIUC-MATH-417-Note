...
\subsubsection{Lemma 1.1.7: injective/surjective/bijective is preserved in composition}
...
\subsection{Equivalence relations (rational equivalence in micro)}
\textbf{rational equivalence in micro} Satisfy: (1)Reflexive, (2)Symmetric, (3)Transitive.
...
\end{example}
\subsubsection{Proposition 1.2.12: use $X/\sim=\{[x]|x\in X\}$ to infer $\sim_\pi$ equals to $\sim$.}
...
\subsubsection{Theorem: Every permutation can be represented by a product of transpositions (not require to be disjoint)}
\begin{theorem}
...
\subsubsection{Theorem: $\left\langle a^v\right\rangle<\{1,a,a^2,...,a^{n-1}\}$ $\Rightarrow$ $\left\langle a^v\right\rangle=\left\langle a^d\right\rangle, d=gcd(v,n), |\left\langle a^v\right\rangle|=\frac{n}{d}$}
\begin{theorem}
...
\subsubsection{Thm: if $N \lhd G$, "$G/N$ is abelian" $\Leftrightarrow$ "$[G,G] \leq N$"}
\begin{theorem}
...
\subsubsection{Thm: If $G$ acts on $X$, $\phi: G \rightarrow S_X$ as $\phi(g) = \sigma_g$ is a homomorphism (where $\sigma_g(x)=gx$)}
\begin{theorem}
...
\subsection{Orbits}
\subsubsection{Thm: Equivalence Relation: $X$ is a $G$-set, $x_1\sim x_2 \Leftrightarrow x_2=gx_1,\ \exists g\in G$}
\begin{theorem}
...
\subsubsection{Thm: if $X$ is a $G$-set, stabilizer $G_x=\{g\in G\vert gx=x\}$ is subgroup of $G$, $\forall x\in X$}
...
\subsubsection{Orbit-Stabilizer Theorem: $|Gx|=\frac{|G|}{|G_x|}$}
\begin{theorem}
...
\subsubsection{Burnside's Formula: number of orbits in $X$: $r=\frac{1}{|G|}\sum_{g\in G}|X^g|$}
\begin{theorem}
...
\subsubsection{Thm: $R/H$ is a ring for $H=ker\phi$ if operations well defined}
\begin{theorem}
...
\subsubsection{Thm: $(a+H)+(b+H)=(a+b)+H$ well defined $\Leftrightarrow$ $ah\in H,hb\in H,\forall a,b\in R,b\in H$}
\begin{theorem}
...
\subsubsection{Cor: Ideal of field $F$ is $\{0\}$ or $F$}
\begin{corollary}
...
\subsection{Def: Maximal ideal: no other ideal properly contains it}
\begin{definition}
...
\subsubsection{Thm: $R$ comm ring with $1$, $M$ maximal ideal $\Leftrightarrow$ $R/M$ is a field}
\begin{theorem}
...
\subsection{Def: Prime ideal: $ab\in P \Rightarrow $ $a\in P$ or $b\in P$}
\begin{definition}
...
\subsubsection{Thm: $N$ prime ideal $\Leftrightarrow$ $R/N$ is an integral domain}
\begin{theorem}
...
\subsubsection{Cor: maximal ideal $\Rightarrow$ prime ideal}
\begin{corollary}
    Every maximal ideal in a commutative ring $R$ with $1$ is a prime ideal.
\end{corollary}

\subsection{Relation Summary}
\begin{center}
...
\textcolor{gray}{
\subsection{Thm: homomorphism $\phi:\mathbb{Z} \rightarrow R$, $\phi(n)=n\cdot 1$}
\begin{theorem}
...
\end{theorem}
\subsubsection{Cor: Ring $R$ 1. characteristic $n > 1$ $\Rightarrow$ has subring isomorphic
...
\subsubsection{Thm: Field $F$ 1. prime characteristic $p$ $\Rightarrow$ has subfield isomorphic to $\mathbb{Z}_p$ 2. characteristic $0$ $\Rightarrow$ has subfield isomorphic
to $\mathbb{Q}$}
\begin{theorem}
...
\subsection{Def: Pricipal ideal (of comm ring $R$) generated by $a$: $\left\langle a\right\rangle=\{ra|r\in R\}$}
\begin{definition}
...
\subsubsection{Thm: field $F$, every ideal in $F[x]$ is principal}
\begin{theorem}
...
\subsubsection{Thm: principal ideal $\left\langle p(x)\right\rangle\neq \{0\}$ of $F[x]$ is maximal $\Leftrightarrow$ $p(x)$ is irreducible}
\begin{theorem}
...
\end{proof}
\subsection{\underline{Irreducible} Polynomials:}
...
\subsection{Theorem 2.3.6: nonconstant polynomials can be reduced uniquely}
...
\subsection{Thm: Euclidean Algorithm of polynomials}
\begin{theorem}
...
\subsection{Cor: $a$ is a zero of $f(x)$ $\Leftrightarrow$ $(x-a)|f(x)$}
\begin{corollary}
...
\subsection{Cor: Finite subgroup of multiplicative $F\backslash \{0\}$ is cyclic}
\begin{corollary}
...
\end{equation}
\end{example}
\subsubsection{Proposition 2.3.10:}
...
\subsubsection{Corollary 2.3.16(of Euclidean Algorithm): $f$ can be divided into $(x-\alpha)q+f(\alpha)$i.e. if $\alpha$ is a root, then $(x-\alpha)|f$}
...
\subsection{Thm: finite $H,K\leq G$, $|HK|=\frac{|H||K|}{|H\cap K|}$}
\begin{proposition}
    For finite subgroups $H,K\leq G$, define $HK=\{hk:h\in H,k\in K\}$.
    $$|HK|=\frac{|H||K|}{|H\cap K|}$$
\end{proposition}

\subsection{Group action by conjugation}
...
\subsection{Lemma: $K\leq N_G(H)$ $\Rightarrow$ $HK\leq G$}
\begin{lemma}
...
\subsection{Cor: if $H \lhd N_g(H)\leq G$, $\#$ subgroups of $G$ conjugate to $H$ is $[G: N_G(H)]$}
\begin{corollary}
...
\subsection{Center $Z(G)=\{a\in G:ag=ga,\forall g\in G\}=\{a\in G:gag^{-1}=a,\forall g\in G\}$}
Size of orbit of $a$ is $1$ $\Leftrightarrow a\in Z(G)$

\subsection{Class Equation: $|G|=|Z(G)|+\sum_{i=1}^r\frac{|G|}{|C_G(g_i)|}$}
