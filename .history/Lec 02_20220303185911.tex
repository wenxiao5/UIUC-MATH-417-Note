...
$$
\left\{1, r, r^{2}, \ldots, r^{n-1}, s, s r, s r^{2}, \ldots, s r^{n-1}\right\} .
$$
The element $r$ corresponds to rotating the $n$-gon by $\frac{2 \pi}{n}$, while $s$ corresponds to reflecting it across the line $O A_{1}$ (here $O$ is the center of the polygon). So $r s$ mean "reflect then rotate" (like with function composition, we read from right to left).
