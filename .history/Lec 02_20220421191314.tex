...
\end{theorem}
\begin{example}
    Since $\mathbb{Z}/n \mathbb{Z}$ is isomorphic to $\mathbb{Z}_n$ and $\mathbb{Z}_n$ is a field if and only if $n$ is prime. Then we see that maximal ideals are $p \mathbb{Z}$ where $p$ is any positive prime.
\end{example}
