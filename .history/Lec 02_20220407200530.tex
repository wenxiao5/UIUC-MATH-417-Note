...
\begin{theorem}
Let \underline{group $G$ act on the set $X$}, the function $\sigma_g:X \rightarrow X$ defined by $\sigma_g(x)=gx$ for $x\in X$ is a permutation of $X$. Also, the map $\phi: G \rightarrow S_X$ defined by $\phi(g) = \sigma_g$ is a homomorphism with the property that $\phi(g)(x) = gx$.
\end{theorem}
