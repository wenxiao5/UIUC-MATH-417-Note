...
\begin{proof}[Proof method 2]
    ...
    Then we have $\left\langle (x-a)\right\rangle\subseteq ker\phi_a\subsetneq F[x]$, where $\left\langle (x-a)\right\rangle=\{ra|r\in F[x]\}$. Since $x-a$ is irreducible, then $\left\langle (x-a)\right\rangle$ is a maximal ideal of $F[x]$. Then $\{ra|r\in F[x]\}=ker\phi_a$
\end{proof}
...
\subsubsection{Corollary 2.4.16: \textit{bijection} $\mathcal{L} (V,M)\rightarrow M_{m\times n}(\mathbb{F})$}
%一个线性变换对应一个矩阵
...
\subsubsection{Proposition 2.4.19: }
%线性变换矩阵相乘仍为线性变换矩阵
